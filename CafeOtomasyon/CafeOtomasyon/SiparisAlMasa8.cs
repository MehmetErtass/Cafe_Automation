...
{
    public partial class SiparisAlMasa8 : Form
    {
        List<tbl_Masa8> list;
        public SiparisAlMasa8()
        {
            InitializeComponent();
        }
        ...
        private void toplamFiyat()
        {
            if (list.Count > 0)
            {
                var fiyat = db.tbl_Masa8.Sum(x => x.fiyat);
                if (fiyat == 0)
                {
        ...
        private void SiparisAlMasa8_Load(object sender, EventArgs e)
        {
            YemekAdiGoster();
            Goster();
            toplamFiyat();
        ...
        private void btnSiparisSil_Click(object sender, EventArgs e)
        {
            ...
            var sonuc = db.tbl_Masa8.Where(x => x.id == kod).FirstOrDefault();
            id = sonuc.id;
            if (txtId.Text == "")
            {
                MessageBox.Show("Eksik Bilgi Girdiniz.");
