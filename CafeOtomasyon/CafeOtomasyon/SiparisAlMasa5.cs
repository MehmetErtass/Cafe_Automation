...
{
    public partial class SiparisAlMasa5 : Form
    {
        List<tbl_Masa5> list;
        public SiparisAlMasa5()
        {
            InitializeComponent();
        }
        ...
        private void toplamFiyat()
        {
            if (list.Count > 0)
            {
                var fiyat = db.tbl_Masa5.Sum(x => x.fiyat);
                if (fiyat == 0)
                {
        ...
        private void SiparisAlMasa5_Load(object sender, EventArgs e)
        {
            YemekAdiGoster();
            Goster();
            toplamFiyat();
        ...
        private void btnSiparisVer_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    if (MessageBox.Show("Kiralama başlatmak istediğinize emin misiniz?", "Kayıt Ekleme", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        tbl_Masa5 tbl = new tbl_Masa5();
                        tbl.yemekAdi = cboxYemekAdi.SelectedValue.ToString();
                        tbl.adet = txtAdet.Text;
                        tbl.fiyat = Convert.ToInt32(txtFiyat.Text);
                        db.tbl_Masa5.Add(tbl);
                        db.SaveChanges();
                        Goster();
                        YemekAdiGoster();
                        toplamFiyat();
        ...
        private void btnSiparisSil_Click(object sender, EventArgs e)
        {
            ...
            var sonuc = db.tbl_Masa5.Where(x => x.id == kod).FirstOrDefault();
            id = sonuc.id;
            if (txtId.Text == "")
            {
                MessageBox.Show("Eksik Bilgi Girdiniz.");
