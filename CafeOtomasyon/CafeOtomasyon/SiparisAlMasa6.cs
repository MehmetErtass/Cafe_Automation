...
{
    public partial class SiparisAlMasa6 : Form
    {
        List<tbl_Masa6> list;
        public SiparisAlMasa6()
        {
            InitializeComponent();
        }
        ...
        private void toplamFiyat()
        {
            if (list.Count > 0)
            {
                var fiyat = db.tbl_Masa6.Sum(x => x.fiyat);
                if (fiyat == 0)
                {
        ...
        private void SiparisAlMasa6_Load(object sender, EventArgs e)
        {
            YemekAdiGoster();
            Goster();
            toplamFiyat();
        ...
        private void btnSiparisSil_Click(object sender, EventArgs e)
        {
            ...
            var sonuc = db.tbl_Masa6.Where(x => x.id == kod).FirstOrDefault();
            id = sonuc.id;
            if (txtId.Text == "")
            {
                MessageBox.Show("Eksik Bilgi Girdiniz.");
