...
{
    public partial class SiparisAlMasa1 : Form
    {
        List<tbl_Masa1> list;
        public SiparisAlMasa1()
        {
            InitializeComponent();
        }
        ...
        private void Goster()
        {
            list =  db.tbl_Masa1.ToList();
            dgvMasa.DataSource = list;
        }

        private void SiparisAlMasa1_Load(object sender, EventArgs e)
        {
            YemekAdiGoster();
            Goster();
            toplamFiyat();
        ...
        private void toplamFiyat()
        {
            if (list.Count > 0)
            {
                var fiyat = db.tbl_Masa1.Sum(x => x.fiyat);
                if (fiyat == 0)
                {
        ...
        private void btnSiparisSil_Click(object sender, EventArgs e)
        {
            ...
            var sonuc = db.tbl_Masa1.Where(x => x.id == kod).FirstOrDefault();
            id = sonuc.id;
            if (txtId.Text == "")
            {
                MessageBox.Show("Eksik Bilgi Girdiniz.");
